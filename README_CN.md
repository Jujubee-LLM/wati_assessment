...
默认情况下，CLI 使用 Qwen LLM planner；如果没有配置 `DASHSCOPE_API_KEY`，会明确报错，不再静默切到非 LLM fallback。Deterministic fallback 仍保留，但必须显式使用 `--provider fallback`，仅用于离线演示和测试。
如果验收时模型接口较慢，可以调大 `QWEN_TIMEOUT_SECONDS`；planner 会重试临时网络超时，但仍不会静默降级到 deterministic parser。
...
- `City campaign`: 先验证 `city=Jakarta` 联系人，再给该 segment 发送 `flash_sale` broadcast。
- `Contact onboarding`: 创建联系人和 custom attributes，然后添加 tag。
...
默认 Planner 使用 Qwen 把自然语言转换为结构化 JSON plan。Deterministic fallback parser 只用于显式的离线 demo 和测试。LLM 不直接调用 API。
...
2. Planner 使用 Qwen 生成结构化 plan；如果缺少 LLM 凭证会快速失败，而不是静默使用非 LLM parser。
...
系统默认使用 Qwen 做 intent understanding 和 plan generation。
...
Deterministic fallback planner 用于 demo 稳定性和测试。它不是 LLM 路径的替代；正常 LLM-backed review 可运行：
...
- 未知 tool、缺少必填参数、非法手机号、错误的 template/attribute 参数结构都会在执行前 validation failed。
- 批量发送会继续处理独立联系人，并汇总 partial failures。
- API/client errors 会按 step 报告。
- 有副作用的操作必须显式 `--execute`。
- real HTTP adapter 对 attribute search 做 contacts pagination，并对 transient API failures 做 retry。
...
- Malformed plan 的 parameter validation。
- HTTP contact pagination behavior。
- Qwen planner response parsing 和 normalization，且不需要真实网络调用。
...
| 125-155 min | Multi-domain demo scenarios 端到端跑通 |
...
- 完整 WATI API 覆盖：V1 实现 demo 所需 subset，包括 contact create/update、tagging、template messages、broadcasts 和 ticket assignment。
...
- 大规模 queue/rate-limit scheduler：当前 batch handling、contact pagination 和 transient retry support 已展示设计方向。
...
- Qwen LLM integration 是 intent understanding 和 plan generation 的默认路径；offline fallback 必须显式选择，并与 LLM 路径清晰分离。
- 跨 Contacts、Tags、Messages、Broadcasts、Tickets 的 multi-domain API orchestration，README 中每个 demo 场景都跨 domain 或 side-effect tool chaining。
- 默认 dry-run / preview。
- Realistic mock client，并保留可替换真实 API 的 adapter。
- Plan validation、parameter validation、pagination-aware contact lookup、retry 和明确 error handling。
- 符合 3 小时时间盒的 scope。
...
- 增加更完整的 rate-limit queue、rollback 和 operator/template metadata handling。
- 提供轻量 chat web UI。
